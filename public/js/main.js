...
$(document).ready(function () {
    ...
    $('#user-rappels-event').on('click', function () {
        $('#user-rappels').html('');
        getRappels("1")
    });

    $('#user-notifs-event').on('click', function () {
        $('#user-notifs').html('');
        getNotifications("1")
    });

    function getRappels(parametre) {
        //parametre 0 = get only count
        //parametre 1 = get count and data
        ajaxUrl = app_url + 'user-rappels/' + parametre;

        $('#user-rappels-count').html('');
        var dataHref = null;
        $.ajax({
            type: "GET",
            url: ajaxUrl,
            ...
            success: function (data) {
                if (data.success) {
                    console.log(data);
                    if (data.message != 0) {
                        $('#user-rappels-count').html(data.message);
                    }

                    if (data.data instanceof Array && data.data.length > 0 && parametre == 1) {
                        $.each(data.data, function (index, value) {
                            dataHref = '/fiche-details/' + value['id'];
                            $('#user-rappels').append(`<li data-href="` + dataHref + `" id="` + value['id'] + `" class="media" style="cursor: pointer ;">
                            ...
                            <div class="media-body">
                                <a href="`+dataHref+`">- Rappel programmé a ` + value['date_rappel']+`</a> 
                            </div>
                         </li>`);
                        });
                    }
    ...
    function getNotifications(parametre) {
        $('#user-notifs-count').html('');
        var dataHref = null;
        $.ajax({
            type: "GET",
            url: app_url + 'user-notifications/'+parametre,
            //data: $("#fiche_forme").serialize(),
            //cache: false,
            success: function (data) {
                if (data.success) {
                    console.log(data);
                    if (data.message != 0) {
                        $('#user-notifs-count').html(data.message);
                    }
