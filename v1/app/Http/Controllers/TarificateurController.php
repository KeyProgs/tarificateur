...
class TarificateurController extends GlobaleController
{
    ...
    public function Tarificateur($ficheId = null, $formule_id = null)
    {
        ...
        if ($fiche != null && $Formule != null) {
            ...
            $departement = Departement::where('code', '=', (Ville::find($ville_id)->department_code))->first();
            $departement_id = $departement->id;

            if ($departement_id == null) dd('Departement ID Introuvable sur :(' . $ville_id . ') ');
            $department_code = Ville::find($ville_id)->department_code;

            $regime_id = $fiche->personne->regime_id;//regime_id
            //$Formule = new Formule();
    ...
    function regle($gamme_id, $zone_id = null, $annee = null)
    {
        ...
        foreach ($Formules as $formule) {
            $prix_s = DB::table('prix_formules')
                ...
                ->where('regles.annee', @$annee)
                ->select('prix', 'age', 'regles.id as  ', 'zones.id as zone_id')->get();;
        ...
        return view('regles', compact('zone', 'zone_id', 'Gamme', 'zone_id', 'idregles', 'Departements', 'Regimes', 'Departement_zones', 'Formules', 'Regime_Regles'));
    ...
    function regles(Request $request)
    {
        ...
        foreach ($FormulesIds as $FormulesId) {
            ...
            $Prix_formule = Prix_formule::where(['regle_id' => $Regle_Id, 'age' => $Gamme->e_age])->first();
            if ($Prix_formule != null) {
                echo "regle  ($Regle_Id) Trouvé Avec Formule : ($FormulesId)   => Prix $Prix sur id : $Prix_formule->id <bR>";
                $Prix_formule::where(['regle_id' => $Regle_Id, 'age' => $Gamme->e_age])->update(['prix' => $Prix]);
                //$Prix_formule->prix = $Prix;
                //$Prix_formule->save();
                //echo "<br> Prix formule ID : " . $Prix_formule->id . " Valeur de prix Depuis Input " . $Prix . "<br>";
            } else {
                $IDPF = Prix_formule::create(['regle_id' => $Regle_Id, 'age' => $Gamme->e_age, 'prix' => (double)$Prix])->id;
...
//2
            for ($Age = $Gamme->min_age; $Age <= $Gamme->max_age; $Age++) {
                ...
                if ($Prix_formule != null) {
                    echo "regle  ($Regle_Id) Trouvé Avec Formule : ($FormulesId)   => Prix $Prix sur id : $Prix_formule->id <bR>";
                    $Prix_formule::where(['regle_id' => $Regle_Id, 'age' => $Age])->update(['prix' => $Prix]);
                    //$Prix_formule->prix = $Prix;
                    //$Prix_formule->save();
                    //echo "<br> Prix formule ID : " . $Prix_formule->id . " Valeur de prix Depuis Input " . $Prix . "<br>";
                } else {
                    $IDPF = Prix_formule::create(['regle_id' => $Regle_Id, 'age' => $Age, 'prix' => (double)$Prix])->id;
                    echo "regle : ($Regle_Id) Formule : ($FormulesId)   => Prix $Prix sur id : $IDPF <bR>";
                }
