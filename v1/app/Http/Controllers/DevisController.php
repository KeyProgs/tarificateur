...
class DevisController extends GlobaleController
{
    ...
    public function checkDevis($devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id)
    {
        ...
        if ($devis_id != null) {
            ...
            $devis->update(['user_id' => Auth::user()->id, 'mode_id' => $mode_id, 'date_prelevement' => $date_prelevement, 'formule_id' => $formule_id, 'fiche_id' => $fiche->id, 'compte_id' => $compte->id, 'cotisation' => $cotisation, 'reduction' => $reduction, 'numero_contrat' => 0, 'date_debut' => $fiche->date_effet, 'date_fin' => NULL, 'fiche_id' => $fiche->id, 'fiche_etat_id' => 35]);
    ...
    public function envoyerMailDevis($devis_id = null)
    {
        ...
        $fiche = Fiche::find($devis->fiche_id);

        $gamme = Formule::find($devis->formule_id)->gamme;
        $piece_jointes = $gamme->piece_jointes;


        $data = $this->initialisationDevis($devis_id);
        if (Mail::send('devis.devis-infos', ['data' => $data, 'piece_jointes', $piece_jointes], function ($message) use ($data, $piece_jointes) {

            $message->to($data['prospect']->email, $data['prospect']->civilite->valeur . ' ' . $data['prospect']->nom . ' ACS ASSURANCE votre devis')
                ->cc("mhamed.erraji@acsassurance.com")
                ->subject('acsAssurance Votre devis ' . $data['devis']->formule->nom . ' à ' . $data['devis']->cotisation . '/mois');
            foreach ($piece_jointes as $piece) {
                $message->attach('http://crm.acsassurance.com/uploads/pieces-jointes/gammes/'.$piece->url);
            }
            // $message->attach('http://crm.acsassurance.com/img/acs.jpg');

            $message->from('gestion@acsassurance.com', 'GESTION ACS ASSURANCE');
    ...
    {
        $data = $this->initialisationDevis($devis_id);
        return view('devis.devis-infos', compact('data'));
    }
