...
class Gamme extends Model
{
    ...
    public function validate3($data)
    {
        ...
        foreach ($Personnes as $personne) {
            ...
            if ($personne['regime_id'] == 1) { //Alsace Mosel
                ...
                || $data['regime_idP'] == 11 //TNS indépendant
                || $data['regime_idP'] == 7 //Retraité TNS
            ) { // TNS  & ExplAgric
    ...
    public function validate28($data)
    {
        $message = 'Validation Gamme 28 Formule(' . $data['formule_id'] . ') Regime(' . $data['regime_idP'] . ') ... ';
        $reduction=100;
        if ($data['conjoint'] == true) {
            $message .= ' Reduction Couple 10% ';
            $data['reduction_couple'] = 1;
            $data['message'] = ' Reduction Couple 10% ';
            $reduction = 10;
        }
        ...
        if($reduction!= 100) $data['cotisation'] = $PrixFinale - ($PrixFinale * $reduction / 100);
        else $data['cotisation'] = $PrixFinale;
        if ($data['formule_id'] == 43) $data['Frais_Dossier'] = 3.81;
        else $data['Frais_Dossier'] = 4.81;
        $data['cotisation'] = $PrixFinale + $data['Frais_Dossier'] ;


        return $data;
    }

    public function validate29($data)
    {
        $message = 'Validation Gamme 29 Formule(' . $data['formule_id'] . ') Regime(' . $data['regime_idP'] . ') ... ';
        $reduction=100;
        if ($data['conjoint'] == true) {
        ...
        if ($data['formule_id'] == 43) $data['Frais_Dossier'] = 3;
        else $data['Frais_Dossier'] = 3;
        $data['cotisation'] = $PrixFinale + $data['Frais_Dossier'] ;
