...
from fiches f 	where
   ...
   public function getVilles($code_postale = null) {
      ...
      foreach($villes as $ville) {
         $response .= '<option data-value="' . $ville->name . '" value="' . $ville->id . '">' . $ville->name . ' (' . $ville->zip_code . ')' . '</option>';
      }
   ...
   public function getCotRed($FormuleId, $FicheId) {
       ...
       $request=new Request();
      $Cot = app('App\Http\Controllers\TarificateurController')->getPrices($request, $FormuleId, $FicheId);
      $Cot= $Cot->getContent();
   ...
   public function setMessage($type, $num, $operation) {
      return $type . " N° " . $num . " a été " . $operation;
   }
