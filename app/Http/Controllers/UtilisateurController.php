...
//predictif
    ...
    {
        //TODO  Add le champ local_server to  Table Equipe.
        //TODO  Récupérer $Serveur_predictive du  champ local_server from Table Equipe
        $equipes = Auth::user()->equipes[0];
        $equipe = Equipe::find($equipes->equipe_id);
        $local_server = $equipe->local_server;
        $nom_equipe = $equipe->valeur;
        $url = "https://$local_server";

        return view('predictif.agent', compact('local_server', 'nom_equipe'));
    }

//ecoute a froid
    public function ecoutefroid()
    {
        $equipes = Auth::user()->equipes[0];
        $equipe = Equipe::find($equipes->equipe_id);
        $local_server = $equipe->local_server;
        $nom_equipe = $equipe->valeur;
        return view('predictif.ecoute-froid', compact('local_server', 'nom_equipe'));
    }

    //nouvelle fiche forme
    public function nouvelleFicheForm(Request $request)
    {
        $data_predictif = null;
    ...
    public function listeDevisJson($month = null, $year = null)
    {
        ...
        $usersIds = $user->getUsersEquipeByUser($user->id);
        //TODO where id in $usersIds  OrderBy dateDevis Desc


        //dd($usersIds);
        foreach ($usersIds as $id) {
            $user = User::where('id', '=', $id)->first();
            $user_devis = $user->devis;

            foreach ($user_devis as $d) {
                if ($d->fiche == null) $d->fiche = Fiche::find(18);
                if ($d->user == null) $d->user = User::find(0);
                //foreach($simulation->devis as $d) {
                $d->prospect = @$d->fiche->personne->nom . " " . @$d->fiche->personne->prenom;
                $d->user = $d->fiche->user->nom . ' ' . $d->fiche->user->prenom;
                $d->sender = @$d->user->nom . ' ' . @$d->user->prenom;
                $d->fiche_id = $d->fiche->id;
                if ($month != null) {
                    if (strpos($d->created_at, $year . "-" . $month) !== false) {
                        array_push($devis, $d);
                    }
    ...
    {
        $fiche = Fiche::find($id);
        $prospect = Personne::find($fiche->personne->id);
        $new_password = $this->randomPassword();
        $prospect->password = Hash::make($new_password);
        $prospect->save();
        Historique::create(['user_id' => Auth::user()->id, 'fiche_id' => $fiche->id, 'action_id' => 17, 'description' => '', 'vue' => false]);
        return $this->sendResponse($new_password, "");
    }


    //uploads gestion
    public function fichePiecesJointes($fiche_id)
    {
        $fiche = Fiche::findOrFail($fiche_id);
        $types_pieces_jointe = Piece_jointe_type::all();
        return view('utilisateur.fiches.uploads', compact('fiche', 'types_pieces_jointe'));
    }

    public function nouvelleFichePieceJointe(Request $request)
    {
        $fiche = Fiche::findOrFail($request->fiche_id);
        $this->validate($request, [
            'type_id' => 'required|numeric',
            'description' => 'nullable|string',
            'pieces_jointes.*' => 'required|mimes:jpg,jpeg,png,bmp,pdf,txt,word,xlsx,xls,mpga,wav,mkv',
        ]);
        if ($request->hasfile('pieces_jointes')) {
            foreach ($request->file('pieces_jointes') as $file) {
                $attributes = [];
                $fileName = time() . '-' . $file->getClientOriginalName();
                $destinationPath = public_path('/uploads/pieces-jointes/fiches/' . $fiche->id);
                $file->move($destinationPath, $fileName);
                $attributes['url'] = $fileName;
                $attributes['type_id'] = $request->type_id;
                $attributes['fiche_id'] = $fiche->id;
    ...
    public function fichePiecesJointeSuppression($fiche_id, $piece_jointe_id)
    {
        ...
        $fiche = Fiche::whereIn('user_id', $user->getUsersEquipeByUser(Auth::user()->id))
            ->where('id', '=', $fiche_id)
            ->first();
        if ($fiche == null || Auth::user()->role->level < 2) {
            abort(403);
        }
        $piece_jointe = Piece_jointe::findOrfail($piece_jointe_id);
        if ($piece_jointe->fiche_id != $fiche->id) {
            abort(403);
        }
        $piece_jointe->delete();
        Session::flash('message', 'Votre demande a été bien traitée');
