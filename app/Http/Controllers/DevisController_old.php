...
class DevisController_old extends GlobaleController {
   //get devis by id
   public function getDevisById($devis_id) {
      $devis = Devis::find($devis_id);
      //$devis->data_formule = $devis->formule;
      //$devis->data_gamme = $devis->formule->gamme;
      $devis->data_formule = $devis->formule->gamme->compagnie;
      return $this->sendResponse($devis, "");
   }
   ...
   public function checkDevis($num_contrat, $devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id,$send) {
      $fiche = Fiche::findOrfail($fiche_id);
      $compte = Compte::where('fiche_id', '=', $fiche->id)->first();
      if($devis_id != null) {
         //dd($devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id);
         $idDevis = $devis_id;
         ...
         $devis->update([
            ...
            'date_debut' => $fiche->date_effet,
            'date_fin' => NULL,
            'fiche_etat_id' => 35]);
         Historique::create(['user_id' => Auth::user()->id,
            'fiche_id' => $fiche->id, 'action_id' => $action_id,
            'description' => '<a href="/devis/' . $devis->id . '">Devis N° ' . $devis->id . ' a été modifier et envoyé</a>', 'vue' => 1]);
      } else {
         $idDevis = Devis::create(['user_id' => Auth::user()->id, 'mode_id' => $mode_id, 'date_prelevement' => $date_prelevement, 'formule_id' => $formule_id, 'fiche_id' => $fiche->id, 'compte_id' => @$compte->id, 'cotisation' => $cotisation, 'reduction' => $reduction, 'numero_contrat' => $num_contrat, 'date_debut' => $fiche->date_effet, 'date_fin' => NULL, 'fiche_etat_id' => 35])->id;
         Historique::create([
            'user_id' => Auth::user()->id,
            'fiche_id' => $fiche->id,
            'devis_id' => $idDevis,
            'action_id' => $action_id,
            'description' => '<a href="/devis/' . $idDevis . '">' . $this->setMessage('Devis', 'N° ' . $idDevis, 'envoyé') . "</a>", 'vue' => 1]);
      }
      if($send){
   ...
   public function buttonEnvoyerDevis($num_contrat, $devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id,$send) {
      //$this->envoyerMailDevis($devis_id);
      $this->checkDevis($num_contrat, $devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id,$send);
   }

   //Souscrire Button function
   public function ButtonSouscrireDevis($num_contrat, $devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id) {
      $this->checkDevis($num_contrat, $devis_id, $date_prelevement, $formule_id, $fiche_id, $action_id, $reduction, $cotisation, $mode_id,null);
   }
...
//       dd('hiii');
   ...
   public function contratInfosForm(Request $request) {
      ...
      //$has_check_etat (false in tarificateur && true in contrat a valider)
   ...
   public function contratInfos(Request $request,$send = null) {
      ...
      if($request->devis_id == null){
         $rules['compagnie_id'] = 'numeric|required';
         $rules['formule_id'] = 'numeric|required';
         $rules['gamme_id'] = 'numeric|required';
      }
      if($request->has('cotisation')) {
         $rules['numero_contrat'] = 'required';
      ...
      if($validator->fails()) {
         ...
         try {
            ...
            if($compte != null) {
               ...
               //$idDevis = $this->buttonEnvoyerDevis($request->numero_contrat, $devis_id, $request->date_prelevement, $request->formule_id, $request->fiche_id, $action_id, $cotRedArray['Red'], $cotisation, $request->mode_id);
               $this->buttonEnvoyerDevis($request->numero_contrat, $devis_id, $request->date_prelevement, $request->formule_id, $request->fiche_id, $action_id, $cotRedArray['Red'], $cotisation, $request->mode_id,$send);
               ...
               if($modif) {
                  ...
                  //return $this->sendResponse($idDevis, 'Le devis ' . $formule->nom . ' abien été  Modifié');
                  return $this->sendResponse("", $this->setMessage('Devis ', $devis_id, 'Modifié'));
               } else {
                  //return $this->sendResponse($idDevis, 'Le devis ' . $formule->nom . ' abien été envoyé sur l\'adresse :  ' . $fiche->personne->email);
                  return $this->sendResponse("", $this->setMessage('Devis', $devis_id, 'enregistré'));
                  //return $this->sendResponse($idDevis, 'Le devis ' . $formule->nom . ' abien été enregistré');
               }
            } else {
...
//dd($devis);
      ...
      $idDevis = $this->checkDevis( $devis->numero_contrat,$devis_id, NULL, $formule_id, $fiche_id, $action_id, $cotRedArray['Red'], $cotRedArray['Cot'], @$devis->mode_id,null);
