...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        //function save or update (devis + contrat)
        function saveOrUpdateDevis(send) {
            //hide errors messages & tarif alert
            var url = null;
            if (send == true) {
                url = app_url + "set-contrat-infos/send";
            } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if ($.isEmptyObject(data.errors)) {
                        ...
                        swal({
                                title: "votre demande a bien été traitée",
                                text: data.message,
                                type: "success",
        ...
        $("#modal_contrat").on('click', '.btn-save-devis', function () {
            saveOrUpdateDevis(false);
        });
        //pour enregistrer la contrat et envoyer le devis
        $("#modal_contrat").on('click', '.btn-save-send-devis', function () {
            saveOrUpdateDevis(true);
        });
        ...
        //get devis by id in modal contrat
        $('#modal_contrat').on('change', '#devis_id', function () {
            var devis_id = $(this).val();
            if (devis_id != "") {

                $.ajax({
                    url: app_url + "get-devis-by-id/" + devis_id,
                    type: "GET",
                    success: function (data) {
                        console.log(data);
                        if (data.success) {
                            //labels
                            $("#compagnie_id").val(data.data['formule']['gamme']['compagnie']['id']).trigger('change');
                            $('#compagnie_id').attr('disabled', 'disabled');
                            $("#formule_id").html('<option  selected="" value="">' + data.data['formule']['nom'] + '</option>');
                            $('#formule_id').attr('disabled', 'disabled');
                            $("#gamme_id").html('<option selected="" value="">' + data.data['formule']['gamme']['nom'] + '</option>');
                            $('#gamme_id').attr('disabled', 'disabled');

                            $("#cotisation").val(data.data['cotisation']);
                            $("#cotisation").prop('readonly', true);
                            $("#numero_contrat").val(data.data['numero_contrat']);
                            if (data.data['date_prelevement'] != null) {
                                $("#date_prelevement").val(data.data['date_prelevement']);
                            }
                            if (data.data['mode_id'] != null) {
                                $("#mode_id").val(data.data['mode_id']);
                            }
                        }
                    }
                });
                ...
                $("#compagnie_id").val('');
                $('#compagnie_id').removeAttr('disabled');
                $('#formule_id').removeAttr('disabled');
                $('#gamme_id').removeAttr('disabled');
                $("#cotisation").val('');
                $("#cotisation").prop('readonly', false);
                $("#numero_contrat").val('');
            }
        ...
        //modal
        $('.modal-password-launch').on('click', function () {
        ...
        $('.btn-generate-password').on('click', function () {
            var fiche_id = $('#fiche_id').val();
            $.ajax({
                url: app_url + "generate-password/" + fiche_id,
                type: "GET",
                cache: false,
                success: function (data) {
                    $('#new_password').val(data.data);
                }
            });
        });
    })
